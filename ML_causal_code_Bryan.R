...
pacman::p_load(MatchIt, postDoubleR, data.table, glmnet, tidyverse, gbm, randomForest, party, mlr3, mlr3learners, DoubleML,
               paradox, mlr3tuning, e1071, nnet, Metrics, grf, dplyr, haven, sandwich, lmtest)

#loading and a bit of feature engineering

setwd("C:/Users/syedm/Desktop/Causal ML")
data <- read_dta("Round2.dta")
data <- zap_labels(data)
data <- subset(data, hhid != 92)

data <- select(data, cash, credit, info, incentivized, average_food2,  
               average_exp2, average_nonfood2, average_calorie_perday2,
               lit, walls_good, monga, dhaka_remit, dhaka_network, village,
               exp_total_pc_r1, subsistencer1, num_adltmalesr1, num_childrenr1, 
               avgQ13earned, constrainedr1, bankedr1, upazila, migrant)

data <- data[complete.cases(data), ]

mapply(function(x, y) data[x, y] <<- 1, seq(nrow(data)), 
       paste0("upazila_", data$upazila))

data[is.na(data)] <- 0

data <- as.data.frame(data)

eq1 <- average_food2 ~ incentivized + 
  lit + walls_good + monga + dhaka_remit + dhaka_network +
  exp_total_pc_r1 + subsistencer1 + num_adltmalesr1 + num_childrenr1 + 
  avgQ13earned + constrainedr1 + bankedr1 + as.factor(upazila)

reg1 <- lm(eq1, data=data)
summary(reg1)

m1coeffs_cl <- coeftest(reg1, vcov = vcovCL, cluster = ~village)
m1coeffs_cl

TAU <- summary(reg1)$coefficients[2, 1]

X <- select(data, lit, walls_good, monga, dhaka_remit, dhaka_network,
            exp_total_pc_r1, subsistencer1, num_adltmalesr1, num_childrenr1, 
            avgQ13earned, constrainedr1, bankedr1, upazila_11, upazila_12,
            upazila_13, upazila_14, upazila_15, upazila_16, upazila_17,
            upazila_18, upazila_19, upazila_20, upazila_21, upazila_22,
            upazila_23)

X <- as.matrix(X)
Y <- as.matrix(data$average_food2)
W <- as.matrix(data$incentivized)
...
for(i in 1:length(seed)){
  ...
  doubleml_data = DoubleMLData$new(data_dt,
                                   y_col = "average_food2",
                                   d_cols = "incentivized",
                                   x_cols = c("lit", "walls_good", "monga", "dhaka_remit", "dhaka_network", 
                                              "exp_total_pc_r1", "subsistencer1", "num_adltmalesr1", "num_childrenr1", 
                                              "avgQ13earned", "constrainedr1", "bankedr1", "upazila_11", "upazila_12",
                                              "upazila_13", "upazila_14", "upazila_15", "upazila_16", "upazila_17",
                                              "upazila_18", "upazila_19", "upazila_20", "upazila_21", "upazila_22",
...
for(i in 1:length(seed)){
  ...
  fit1 <- lm(Y ~ W + X, data = m.data1, weights = 1/weights)
  ...
  m.out1 <- matchit(incentivized ~ lit + walls_good + monga + dhaka_remit + dhaka_network +
                      exp_total_pc_r1 + subsistencer1 + num_adltmalesr1 + num_childrenr1 + 
                      avgQ13earned + constrainedr1 + bankedr1 + upazila_11 + upazila_12 +
                      upazila_13 + upazila_14 + upazila_15 + upazila_16 + upazila_17 +
                      upazila_18 + upazila_19 + upazila_20 + upazila_21 + upazila_22 +
                      upazila_23, data = data,
  ...
  ps.xgb = ps(incentivized ~ lit + walls_good + monga + dhaka_remit + dhaka_network +
                exp_total_pc_r1 + subsistencer1 + num_adltmalesr1 + num_childrenr1 + 
                avgQ13earned + constrainedr1 + bankedr1 + upazila_11 + upazila_12 +
                upazila_13 + upazila_14 + upazila_15 + upazila_16 + upazila_17 +
                upazila_18 + upazila_19 + upazila_20 + upazila_21 + upazila_22 +
  ...
  design.ps<-svydesign(ids=~1,weights=~1/weights_xgb,data=data) #ids is for clustering variable
  
  output1<-svyglm(average_food2 ~ incentivized + lit + walls_good + monga + dhaka_remit + dhaka_network +
                    exp_total_pc_r1 + subsistencer1 + num_adltmalesr1 + num_childrenr1 + 
                    avgQ13earned + constrainedr1 + bankedr1 + upazila_11 + upazila_12 +
                    upazila_13 + upazila_14 + upazila_15 + upazila_16 + upazila_17 +
                    upazila_18 + upazila_19 + upazila_20 + upazila_21 + upazila_22 +
                    upazila_23, design=design.ps)
